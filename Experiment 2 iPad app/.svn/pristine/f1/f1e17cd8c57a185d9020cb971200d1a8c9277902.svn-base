...
//        blockCounter = state.block;
        if (log != nil)
        {
            ...
            currentCondition = [[currentBlock objectForKey:@"condition"] valueForKey:@"name"];
...
- (void) startNewBlock:(NSDictionary *)blockInfo
{
    ...
    currentBlock = [[NSMutableDictionary alloc] init];
    repetitions = [[NSMutableArray alloc] init];
    currentCondition = [[blockInfo valueForKey:@"name"] retain];
    NSLog(@"logger: name of current condition : %@", currentCondition);
...
- (void) additionalQuestions:(NSDictionary *)addInfo
{
    [currentQuestion setObject:[addInfo retain] forKey:@"additionalInfo"];
}
