...
#define NUM_REPETITIONS 2 // there's another NUM_REPETITIONS in state machine class
...
@synthesize state, pageCounter, trainingFrame, currentDataViewController, userInfoView, currentConditionName;
...
//    [[UIApplication sharedApplication] setStatusBarHidden:YES animated:NO];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(nextQuestion:) name:@"additionalQuestionsFinished" object:nil];
...
//    }
    ...
    blockFinished = [[UIAlertView alloc] initWithTitle:@"Condition finished" message:@"You can rest now. Press OK when you are ready to continue." delegate:state cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
    blockFinished.tag = 2;
    
    trialFinished = [[UIAlertView alloc] initWithTitle:@"Data set finished" message:@"You can rest now. Press OK when you are ready to continue." delegate:state cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
    trialFinished.tag = 3;
    ...
    trainingFinishedAlert = [[UIAlertView alloc] initWithTitle:@"Training finished." message:@"You can rest if you like. Press start when you are ready to begin." delegate:state cancelButtonTitle:@"Start" otherButtonTitles:nil, nil];
    trainingFinishedAlert.tag = 3;
...
//        }
//    }
    self.pageViewController = [[[UIPageViewController alloc] initWithTransitionStyle:nil /*UIPageViewControllerTransitionStylePageCurl*/ navigationOrientation:UIPageViewControllerNavigationOrientationHorizontal options:nil] autorelease];
    ...
    ((DataViewController*) startingViewController).questionNumber = pageCounter+1;
...
- (DataViewController*) nextView
{
    ...
    [self.pageViewController setViewControllers:viewControllers direction:UIPageViewControllerNavigationDirectionForward animated:NO completion:NULL];
    ((DataViewController*) nextViewController).questionNumber = pageCounter+1;
...
{
    [logger setUserInfo:[[NSDictionary dictionaryWithObjectsAndKeys:[userID text], @"userID", nil] retain]];
...
//        [questionFinished show];
    ...
    {    
        ...
        {
            training = NO;
            [state closeCurrentModel];
            [trainingFinishedAlert show];
        }
        else if (state.repetition + 1 < NUM_REPETITIONS)
        {
            [state closeCurrentModel];
            [trialFinished show];
        }
